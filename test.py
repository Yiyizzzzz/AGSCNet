...
if __name__ == "__main__":
    ...
    # load model
    net.load_state_dict(torch.load('AGSCNet.pth', map_location=device))
    net.eval()
    ...
    with tqdm(total=len(img_path), desc='Test Epoch #{}'.format(num), ncols=160, colour='cyan') as t:
        for tests_path, label_path in zip(img_path, label_path):
            ...
            save_res_path = tests_path.split('\\')[-1].split('.')[0] + '_res.tif'
            save_res_path = '.' + save_res_path.replace('image', 'results')
            name = tests_path.split('\\')[-1].split('.')[0]
            ...
            test_img = test_img.unsqueeze(0)[:1]
